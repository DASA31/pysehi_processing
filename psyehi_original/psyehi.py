...
def process_files(files:str or dict, AC:bool=True, condition_true:list=None, condition_false:list=None, register=True, custom_name=None, colour_thirds=None, overview_img:str=None, is_zeiss:bool=False):
    ...
    output.summary_excel(data_pro_path, condition_true, condition_false, custom_name=custom_name, is_zeiss=is_zeiss)
    print("Excel summary")
    if overview_img is not None:
        output.location_mosaic(data_pro_path, overview_img, condition_false)

def list_files(path_to_files, date:int=None, condition_true:list=None, condition_false:list=None, load_data=False, uint8=False, custom_name=None, is_zeiss:bool=False):
    """
    List raw files for processing or processed files for analysis.

    Parameters:
        date (int): date in YYMMDD format
        path_to_files (str): path to raw or processed files from a certain date
        condition_true (list): list of strings that must be included in root for root to be added to list of files
        condition_false (list): list of strings that must not be included in root for root to be added to list of files
        load_data (bool): if True, will add asociated .tif stack to dict

    Returns:
        data_files (dict): dictionary of data files with keys such as date, material...
    """
    data_files={}
    for root, _, file_list in os.walk(path_to_files):
        if len(file_list) > 0:
            ...
            for part in pathlib.Path(root).parts:
                ...
                ## NEW
                date = regex.search(r"(\d{6}[\w-]*)", part)
                if date is not None:
                    date=date.group(0)
                    break
            ...
            if 'Raw' in path_to_files:
                ...
                if any('Log.csv' in file for file in file_list) or any(file_str in file for file in file_list):
                    ...
                    ## NEW
                    if root.find(date) < root.find('Raw'):
                        material = pathlib.Path(root).parts[
                            pathlib.Path(root).parts.index(date) - 1
    ] # folder BEFORE date
            ...
            if 'Processed' in path_to_files:
                if any('stack' in file for file in file_list) and not any(f in root for f in ['Metadata','Colour','colour','ROI']):
                    ...
                    if load_data:
                        data_files[rf'{date}_{name}']['data'] = data(data_files[rf'{date}_{name}']['Processed_path'], force_uint8=uint8, is_zeiss=is_zeiss)
                    else:
                        data_files[rf'{date}_{name}']['data']['stack_meta'] = data(data_files[rf'{date}_{name}']['Processed_path'], is_zeiss=is_zeiss).stack_meta
...
def load(folder, AC=True, register=True, calib=None, uint8=False, is_zeiss=False):
    """
    Load a folder (path_to_file) containing a SEHI/LL-BSE Hyperspectral Imaging data volume.

    Parameters:
        folder (str):  path_to_files.
        AC (bool, optional): Whether to do angular correction if there is an associated '{folder}_R' dataset. The default is True.
        register (bool, optional): Whether to register the SEHI data volume using image features and normalised coefficient correlation. The default is True.
        calib (str, optional): If the system is a Helios SEM, the calibration.csv file can be specified. The default is None.
        ...
        uint8 (bool, optional): If True, convert stack data to uint8 dtype. Default is False which preserves original dtype.
    
    Returns:
        stack (numpy.ndarray): numpy array of dtype of the original images.
        stack_meta (dict): dictionary of original image metadata. Also saved to stack_meta.json file.
        eV (nump.ndarray): numpy array (float64) of eV values (from energy convertion of deflector voltages).
        dtype_info (numpy.iinfo): stack and original image dtype.
        name (str): name of data folder (leaf name).
        coeffs (numpy.ndarray): numpy array (float64) of coefficients for energy conversion from MV
    """
    ...
    if 'Processed' in folder:
        ...
        if is_zeiss:
            if sys==True:
                ...
                coeffs= np.array(1) # Need proper calibration
                # eht_target = stack_meta[page]['Beam']['HV']
                eV = np.array((ana_voltage*coeffs))
                # NOTE - How should the coeffs be treated for Zeiss data? 
                # As backscattered, assume linear conversion of EHT Target - ESB Grid = eV
        else:
            if sys == True:
                for page in stack_meta:
                    ana_voltage.append(stack_meta[page]['TLD']['Mirror'])
    ...
    if 'Raw' in folder:
        ...
        files = glob.glob(rf'{folder}{slash}*.tif*') # match for both tif and tif (and tifxxxx)#[::4]
        ...
        if 'Helios' in metadata['System']['SystemType']:
            ...
            files_sorted = files
        elif 'Zeiss' in metadata['System']['SystemType']:
            analyser = 'Deflector'
            sys = True # Makes sure images are in correct order based on analysing voltage
            for file in files:
                metadata = load_single_file(file, load_img = False)
                ana_voltage.append(metadata['TLD']['Mirror']) # NOTE I assume esb grid is the analyser voltage of interest
            else:
                print('neither Nova or Helios data')
            if sys == True and not is_zeiss:
                files_sorted = [files for (ana_voltage,files) in sorted(zip(ana_voltage,files), key=lambda pair: pair[0], reverse=sys)]
                ana_voltage = []
            if is_zeiss:
                # will reverse sort based on analyser voltage
                files_sorted = [files for (ana_voltage,files) in sorted(zip(ana_voltage,files), key=lambda pair: pair[0], reverse=sys)]
                ana_voltage = []
        ...
        imgs = []
        stack_meta = {}
        ref_img = tf.imread(files_sorted[-1]) # reference image 
        ...
        for i,file in enumerate(files_sorted):
            if is_zeiss:
                i+=1 # Zeiss files ordered from 0 # Changed to be consistent with Matlab indexing
            else:
            ...
            if register is True:
                ...
                # reg_img, shift_y, shift_x = align_img_pcc(ref_img, img, crop_y=None, crop_x=None, upsample_factor = 10)
                # reg_img, shift_y, shift_x = align_img_transform(ref_img,img)
            ...
            if register is False:
                imgs.append(img)
            if sys == True:
                print(i, metadata['TLD'][analyser]) # Print V to track progress
        ...
        if is_zeiss:# NOTE - How should the coeffs be treated for Zeiss data? 
            if sys==True:
                # As backscattered, assume linear conversion of: EHT Target - ESB Grid = eV
                coeffs=np.array(1) # Placeholder - need proper calibration?
                # eht_target = metadata['Beam']['HV']
                # eV = np.array(eht_target - (ana_voltage*coeffs))
                eV = np.array((ana_voltage*coeffs))
            else:
...
def reformat_zeiss_metadata(zeiss_metadata):
    ...
    zeiss_metadata['Scan']['PixelWidth'] = zeiss_metadata['ap_pixel_size'][1] / 1e9 # in nm to m  # Could also use ap_image_pixel_size which should be identical
    ...
    zeiss_metadata['EScan']['PixelWidth'] = zeiss_metadata['ap_pixel_size'][1] / 1e9 # in nm to m # Could also use ap_image_pixel_size which should be identical
...
def sys_type(metadata):
    """
    Gives system type of the ZeissSmartGemini/FEI/ThermoFisher microscope and analyser type 'Mirror'/'Deflector' for Zeiss/FEI/ThermoFisher.

    Parameters:
        metadata (python dict): A single page of Zeiss image metadata

    Returns:
        sys, True for Zeiss, True for Helios, False for Nova
    ...
    if 'Zeiss' in metadata['System']['SystemType']:
        ...
        analyser = 'Deflector'  # Treat like this for now?
    return sys, analyser
...
def align_img_transform(ref_img, mov_img): 
    im1 = ref_img.astype(np.float32)
    im2 = mov_img.astype(np.float32)
    h, w = ref_img.shape
...
def MV(stack_meta):
    """
    Return EsB grid voltages from stack metadata.

    Parameters:
        stack_meta (dict): dictionary of original image metadata. Also saved to stack_meta.json file.

    Returns:
        MV (numpy.ndarray): array of float64.
    """
    MV = []
    for page in stack_meta:
...
def plot_axes(norm=False, x_eV=True):
    ...
    Parameters:
        ...
        x_eV (bool, optional): If True plots eV values. If False, plots MV/ESB values with inverted axis. The default is True.
...
def spec_dose(stack_meta):
    """
    Returns the electron dose per field of view spectrum or LL-BSE hyperspcetral imaging data volume expressed through an overall charge exposure, given by the product of the beam current I_0 and the total exposure time t_total per area A, where units are coulombs per metre squared [Cm^-2].

    Parameters:
        stack_meta (python dict): dict of Zeiss metadata throughout stack.

    Returns:
        Float: Spectrum dose [Cm^-2]. 
    """
...
def load_roi_file(path_to_roi_file):
    ...
    Parameters:
        path_to_roi_file (str): path to imageJ .roi file or .zip of files.

    Returns:
        rois (dict): dict of rois with properties such as type, xy_crop.
    """
...
class data:
    ...
    def rows(self, xlim=[0,7], x_eV=True): # Change xlim=[]
        """
        Return the indexes of values within eV ranges defined by xmin and xmax.

        Parameters:
            xlim (list, optional): [xmin,xmax] or 'all'. The default is [0,50].
            x_eV (bool, optional): True gives eV indexes, False gives MV indexes. The default is 50.

        Returns:
            rows (numpy.ndarray): the indexes of values that lie within xmin and xmax unless span is false.
        """
        if x_eV is True:
            x = self.eV
        if x_eV is False:
    ...
    def plot_spec(self, rois=None, groups:dict or str=None, plot=True, x_eV=True, xlim=[-1,8], pixel_spec:int=None,
        ...
        """
        Parameters:
            rois (TYPE, optional): str or dict from roi_masks function. The default is None.
            groups (dict, optional) : To provide average plot from grouped areas.
                eg. groups={'PVDF':['1a','1b','1c','1d'],'Gr':['2a','2b','2c','2d'],'CB':['3a','3b','3c']}.
                The default is None.
            plot (TYPE, optional): DESCRIPTION. The default is True.
            x_eV (TYPE, optional) : DESCRIPTION. The default is True.
            xlim (TYPE, optional): 
            pixel_spec (int, optional): DESCRIPTION. The default is None.
            smooth_width (int, optional): DESCRIPTION. The default is None.
            savefig (TYPE, optional): DESCRIPTION. The default is False.
